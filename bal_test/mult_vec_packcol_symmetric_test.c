...
void mult_vec_packcol_symmetric_test()
{
	double **matriz = NULL;
	sp_coord* sp = NULL;
	sp_packcol* sp1 = NULL;
	...
	printf(" -- mult_vec_packcol_symmetric_test -- \n");
	...
	if (bal_cargar_matriz("./data/matriz_sim_6x6.m", &matriz, &n, &m) == 0) {
		...
		bal_mult_vec_packcol_symmetric(sp1, x, y);
		...
		printf("mul_vec_packcol_symmetric_test: Error al leer el archivo\n");
}
